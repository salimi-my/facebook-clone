...
function StoryCard({ user, name, story, profile }) {
  ...
  let createStory = (
    <div className='relative w-28 md:w-32 shadow-md ease-in transition hover:brightness-75 rounded-b-md md:rounded-b-xl overflow-x'>
      <div
        className='relative h-24 min-w-full rounded-t-md md:h-44 cursor-pointer overflow-x 
  p-3 tranform ease-in md:rounded-t-xl'
      >
        <Image
      ...
      <div
        className='bg-white h-12 min-w-full rounded-b-md md:h-12 overflow-x 
  p-3 tranform ease-in md:rounded-b-xl'
      ></div>
    </div>
  );
